...
F.resetWin = function(){
	 ...
	 var WW = $(window).width();
	 var LW = $("#main_left").outerWidth();
	 var CW = $("#main_center").outerWidth();
	 var RW = WW-LW-CW;
	 RW = RW>0?RW:0;
...
F.initHeaderNav = function(){
     var Link = $("#header .nav span");
	 Link.click(function(){
		 var mid = $(this).data('id');
		 $("#menu-tree .mtree").each(function(index,ele){
		     var id = $(this).data('id');
			 var Display = id==mid?'block':'none'; 
...
F.initMenuTree = function(){
     var Tree = $("#menu-tree");
	 var TreeDt = Tree.find(".mtree dt");
	 Tree.find("dl").each(function(){
	     $(this).find("dd:last").addClass("last");
	 });
	 TreeDt.click(function(){
	     var t = $(this).hasClass('open');
		 if(t){
		     $(this).removeClass('open').nextAll().hide();
		 }else{
		     $(this).addClass('open').nextAll().show();
		 }
	 });
}


F.openDialog = function(opts){
     return $.fdialog(opts);
}


F.makePicHtml = function(data,pagesize){
...
F.makeClassifyHtml = function(data,pagesize,pagemax){
...
F.makePagesHtml = function(data,pagesize){
